...
defc	NSTARS	= 32		; number of stars per plane
...
screen_to_show:
	defb 0		; 0: title screen; 1: hi scores; 2: credits
timer:
	defb 0
...
readloop:
	push bc
	push hl
	call _SCAN_KEYBOARD		; read keyboard in A
	pop hl
	pop bc
	cp  13				; 13 is ENTER
	jr nz, read_noenter
...
PUBLIC _showhiscores
_showhiscores:
	; load title screen
	ld hl, _title_hiscores
	ld de, 16384
	call depack
...
defc	JOY_KEMPSTON	= 0
defc	JOY_SINCLAIR1	= 1
defc	JOY_SINCLAIR2	= 2
defc	JOY_KEYS		= 3
...
anim_order:
	...
	defb 	2,2,3,3,3,3,3,3
	defb	3,3,3,3,3,3,3,3
	defb	3,3,3,3,3,3,3,3
	defb	2,2,2,2,2,2,1,1
	defb	1,1,0,0,0,0,0,0
	defb	0,0,0,0,0,0,0,0
	defb	0,0,0,0,0,0,0,0
...
string_silence:
	defb 91,91,"silence",91,91,91,0
string_music_and_fx:
	defb "music",91,"and",91,"fx",0
string_music:
	defb 91,"music",91,"only",91,0
string_fx:
...
fire_pressed_wait:
	defb 0			; If we have pressed fire, wait for two seconds before exiting (to allow effect to play)
sound_selection:
